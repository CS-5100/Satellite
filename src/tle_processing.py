...
def tles_to_dataframe(
    ...
    Args:
        raw_tle_list (list(str)): A list of strings derived either from convert_TLE_text_file_to_list or download_current_tles_as_list
        time (datetime, optional): The datetime that the data is valid for. Defaults to datetime.now(timezone.utc).
        filter_dtc (bool, optional): Whether or not the DTC satellites should be filtered out of the dataset. Defaults to True.
        calculate_radii_areas (bool, optional): Whether or not to calculate the area covered by each satellite and radius of coverage. Defaults to False.
        angle (float, optional): If calculate_radii_areas is true, the angle is the coverage angle of the satellite (the angle at the top of a right cone). Defaults to 45.0.
    ...
    Returns:
        pandas.DataFrame:
            a pandas DataFrame object with the following columns
                ...
                - Radius (float, optional): the radius of the circle covered by the satellite on Earth in kilometers
                - Area (float, optional): the area covered by the satellite on Earth in square kilometers
    """
...
def tle_dataframe_to_geodataframe(tle_dataframe: pd.DataFrame, crs = "EPSG:4326"):
    """Takes a pandas DataFrame object generated by tles_to_dataframe and converts it
    to a geopandas GeoDataFrame with a Point object for every satellite and 

    Args:
        tle_dataframe (pd.DataFrame): a pandas DataFrame object with data derived from a list of TLEs
        crs (str, optional): Any specification for a geodetic coordinate reference system. Defaults to "EPSG:4326".
