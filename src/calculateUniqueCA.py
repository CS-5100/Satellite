...
# ----------------------------------------------------------
# Step 1: Load and process the satellite data
# ----------------------------------------------------------
...
# Save the updated DataFrame with the coverage radius to a new CSV file
satellite_data.to_csv('updated_satellite_coverage.csv', index=False)
...
# We get an error here because BallTree by default supports Euclidean distance over haversine.
...
def find_neighbors(tree, index, radius):
    ...
    indices = tree.query_radius(query_point,
                                r=radius / 6371.0)  # Convert radius to radians for Earth's surface
...
# Initialize an empty graph where nodes represent satellites
# Edges represent overlaps between their coverage areas
...
for i, sat1 in satellite_data.iterrows():
    ...
    for neighbor in neighbors:
        if neighbor != i:  # Avoid connecting the satellite to itself
            ...
            distance = haversine(sat1['Latitude'], sat1['Longitude'], sat2['Latitude'],
                                 sat2['Longitude'])
...
# Get the connected components (groups of overlapping satellites)
...
def calculate_group_union_area(group, satellite_data):
    ...
    # Return the total unique area (subtracting overlaps from the total area)
