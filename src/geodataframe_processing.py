    ...
    Returns:
        gpd.GeoDataFrame:
            a geopandas GeoDataFrame with the following parameters
                    ...
                    - new_satellite (bool): whether or not the satellite is a new candidate satellite or an existing satellite.
                        Satellites generated by this function are not new candidates.
    """
...
def generate_new_satellites(
    ...
    """Generates a GeoPandas GeoDataFrame object describing the x, y coordinates of a set of new
    candidate satellite positions. Does not generate altitudes for these satellites.

    Args:
        EPSG (int | str): the EPSG code specifying the coordinate reference system to project the satellite data to
        num_satellites (int, optional): Number of new candidate satellites to generate. Defaults to 60.
        input_map (_type_, optional): If specified, the vector map data defining the geometric shape
            from which to randomly sample coordinate points from. Defaults to None.
        true_random (bool, optional): Whether or not to sample randomly from valid latitudes and longitudes not using map data. Defaults to True.
        sample_separate (bool, optional): If map data is supplied in a set of independent polygons, setting this as true samples points from each polygon separately.
            Setting this to False merges all the map data into one polygon and samples from the entire map randomly. Defaults to True.
        random_seed (int, optional): The random seed to use for the random number generator in the case that map data is not used for satellite generation.
            Defaults to None.
    ...
    Returns:
        gpd.GeoDataFrame:
            a geopandas GeoDataFrame with the following parameters
                    ...
                    - geometry (shapely Point): the shapely Point object representing the underlying satellite position
                    - new_satellite (bool): whether or not the satellite is a new candidate satellite or an existing satellite.
                        Satellites generated by this function are new candidates so this value is true for all entries.
    """
